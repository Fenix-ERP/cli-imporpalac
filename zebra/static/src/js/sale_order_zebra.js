...
async function setupQzSecurity() {
    qz.security.setCertificatePromise((resolve, reject) => {
        fetch(QZ_CERTIFICATE_URL)
            .then((response) => {
                if (!response.ok) {
                    throw new Error(`Failed to load certificate. Status: ${response.status}`);
                }
    ...
    qz.security.setSignaturePromise((toSign) => {
        return (resolve, reject) => {
            fetch(`${QZ_SIGNATURE_URL}/${encodeURIComponent(toSign)}`)
                .then((response) => {
                    if (!response.ok) {
                        throw new Error(`Signature request failed. Status: ${response.status}`);
                    }
                    return response.text();
                })
                .then((signature) => {
...
actionRegistry.add("zebra_print_action", async (env, action) => {
    ...
    try {
        ...
        const zebraPrinter = availablePrinters.find(printer => printer === "Zebra_ZD220");
