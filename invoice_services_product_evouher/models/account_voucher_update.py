...
class ElectronicVoucherUpdate(models.Model):
    ...
    @api.depends('lines_ride.product_id', 'lines_ride.product_id.type', 'vendor_id.category_id')
    def _compute_product_type_selection(self):
        ...
        for i, record in enumerate(self, 1):
            try:
                ...
                if not record.lines_ride or record.document_type not in ['factura', 'notasdecredito']:
                ...
                has_almacenable_tag = any(tag in vendor_categories for tag in almacenable_tags)
                has_servicio_tag = any(tag in vendor_categories for tag in servicio_tags)
        ...
        _logger.info("✅ Finalizado _compute_product_type_selection para %s registros", total_records)